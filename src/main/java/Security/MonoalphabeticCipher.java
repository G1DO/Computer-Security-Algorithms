...
public class MonoalphabeticCipher {
   ...
   /* private static Map<Character, Character> generateEncryptionMap(String key) {
    ...
    public static String encrypt(String plaintext, String key) {
        ...
        for (char c : plaintext.toCharArray()) {
            // Check if the character is in the encryption map (i.e., A-Z)
 ...
 */
    public static String decrypt(String ciphertext, String key) {
        ...
        for (char c : ciphertext.toCharArray()) {
            // Check if the character is in the decryption map (i.e., a cipher letter)
    ...
    public static String findKey(String plaintext, String ciphertext) {
        ...
        for (int i = 0; i < plaintext.length(); i++) {
            ...
            if (Character.isLetter(plainChar)) {
                int index = alphabet.indexOf(plainChar);
                // Ensure each letter is mapped only once
                if (keyMap[index] == ' ') {
                    keyMap[index] = cipherChar;
                } else if (keyMap[index] != cipherChar) {
                    // Inconsistent mapping: same plaintext letter maps to different ciphertext
                    throw new IllegalArgumentException("Inconsistent mapping detected");
                }
            }
        }
